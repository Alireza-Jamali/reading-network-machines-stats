...
public class XClientInfo {
    ...
    private static final String SERVER = "192.168.7.183";

    public XClientInfo() {
    }

    public static void main(String[] args) {
        XClientInfo xci = new XClientInfo();
        XServer xServer = new XServer("192.168.7.183");

        Thread serverThread = new Thread(xServer);
        serverThread.start();

        xci.pingThread().start();
    ...
    private void sendInfo() {
        ...
        for (boolean b = true; b;) {
            try {
                ...
                try {
                    socketChannel.connect(new InetSocketAddress("192.168.7.183", 1234));
    ...
    private String getHardwareInfo() {
        ...
        try {
            ...
            stb.append(ip.getHostName() + "(XClient)" + "=");
            ...
            public void run() {
                try {
                    for (;;) {
                        ...
                        Throwable localThrowable2 = null;
                        try {
                            ssc.bind(new InetSocketAddress(77));
                            ssc.accept();
                        } catch (Throwable localThrowable1) {
                            localThrowable2 = localThrowable1;
                            throw localThrowable1;
